function [W, K, elapse] = constructW(fea,options)
%	Usage:
    ...
    if strcmpi(options.Metric,'Euclidean')
        ...
        for i = 1:ceil(nSmp/nBlock)
            if i == ceil(nSmp/nBlock)
                ...
                G((i-1)*nBlock*(options.k+1)+1:nSmp*(options.k+1),1) = repmat(smpIdx',[options.k+1,1]);
                G((i-1)*nBlock*(options.k+1)+1:nSmp*(options.k+1),2) = idx(:);
                G((i-1)*nBlock*(options.k+1)+1:nSmp*(options.k+1),3) = exp((dump(:)-1)/(options.sigma^2));
            else
                smpIdx = (i-1)*nBlock+1:i*nBlock;
                dist = fea(smpIdx,:)*fea';
                dist = full(dist);
                ...
                G((i-1)*nBlock*(options.k+1)+1:i*nBlock*(options.k+1),1) = repmat(smpIdx',[options.k+1,1]);
                G((i-1)*nBlock*(options.k+1)+1:i*nBlock*(options.k+1),2) = idx(:);
                G((i-1)*nBlock*(options.k+1)+1:i*nBlock*(options.k+1),3) = exp((dump(:)-1)/(options.sigma^2));
            end
        end
